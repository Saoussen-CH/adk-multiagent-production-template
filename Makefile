...
test-tools: ## Run pure tool tests (no LLM, mocked Firestore) — fast
	$(COMMON_ENV) $(PYTEST) \
		...
		tests/unit/test_fixtures_tenancy.py \
		...
		tests/unit/test_backend_packaging.py \
		tests/unit/test_tool_response_fields.py \
...
setup-gateway: ## Create egress Agent Gateway + Registry entries for FedEx MCP (use ENV=staging|prod)
	$(eval ENV_FILE := $(if $(ENV),.env.$(ENV),.env))
